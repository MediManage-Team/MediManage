...
Auto-detects: NVIDIA GPU (CUDA), AMD GPU (DirectML), CPU.
...
def detect_hardware():
    ...
    Returns dict with:
        ...
        - device_name: e.g. "NVIDIA RTX 4090" or "AMD Radeon RX 7900"
    ...
    # 2. Check AMD GPU (DirectML on Windows)
    amd = _detect_amd()
    if amd:
        return {
            "backend": "directml",
            ...
            "reason": f"AMD GPU detected: {amd['name']} (DirectML)"
        }

    # 3. CPU fallback (BitNet.cpp / llama.cpp)
    cpu = _get_cpu_info()
    return {
        "backend": "cpu",
        ...
        "vram_mb": 0,
        "ram_mb": cpu["ram_mb"],
        "reason": f"No GPU found. Using CPU: {cpu['name']} (BitNet.cpp)"
    }
